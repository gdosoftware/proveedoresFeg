...
<c:set var="entity"  value='<%=request.getAttribute(object).getClass().getSimpleName().toLowerCase()%>' 
       scope="request"/>
<%--guardo bajo el nombre commandname y lo hago persisitir en el request para que en el formfield lo puedan 
utilizar y no tenr que pasar un object
<c:set var="commandName" value="${requestScope[object]}" scope="request"/>--%>
